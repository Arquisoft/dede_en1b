...
export const baseApiEndPoint = window.location.href.includes("dedeen1b.tk") ? "https://api.dedeen1b.tk" : (process.env.REACT_APP_API_URI || 'http://localhost:5000');
...
export async function addOrderToUser(webId: string) {
  ...
  let response = await fetch(apiEndPoint + '/order', {
    ...
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({
      'userId': webId, 'deliveryPrice': shippingCost, 'products':
